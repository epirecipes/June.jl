#!/usr/bin/env python3
"""
SIR Large scenario — Python side.

Same as simple SIR but with 2000 people for scaling/performance testing.
2 schools, 2 companies.

Usage:
    python eval/scenarios/sir_large.py --seed 42 --n_people 2000 --n_steps 100
...
def run_sir_large(seed, n_people=2000, n_steps=100, n_initial_infected=25,
    ...
    school_eligible = [i for i in range(n_people) if people_age[i] <= 18]
    company_eligible = [i for i in range(n_people) if people_age[i] > 18]

    school1 = [i for j, i in enumerate(school_eligible) if j % 2 == 0]
    school2 = [i for j, i in enumerate(school_eligible) if j % 2 == 1]
    company1 = [i for j, i in enumerate(company_eligible) if j % 2 == 0]
    company2 = [i for j, i in enumerate(company_eligible) if j % 2 == 1]

    all_groups = households + [school1, school2, company1, company2]
...
def main():
    parser = argparse.ArgumentParser(description="SIR Large scenario (Python)")
    parser.add_argument("--seed", type=int, required=True)
    parser.add_argument("--n_people", type=int, default=2000)
