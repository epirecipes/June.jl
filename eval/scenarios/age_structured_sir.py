#!/usr/bin/env python3
"""
Age-Structured SIR scenario — Python side.
...
Usage:
    python eval/scenarios/age_structured_sir.py --seed 42 --n_people 200 --n_steps 50
...
def age_multiplier(age):
    """Transmission multiplier based on susceptible person's age."""
    if age <= 18:
        return 1.5
    elif age <= 64:
        return 1.0
...
def run_age_structured_sir(seed, n_people=200, n_steps=50, n_initial_infected=5,
                ...
                if infected[pid] and not recovered[pid]:
                    ...
                    for qid in group:
                        if not infected[qid] and not recovered[qid] and not new_infections[qid]:
                            # Multiply by age-dependent susceptibility of target
                            prob = base_prob * age_multiplier(people_age[qid])
                            if rng.random() < prob:
...
def main():
    parser = argparse.ArgumentParser(description="Age-Structured SIR scenario (Python)")
    ...
    results = run_age_structured_sir(
