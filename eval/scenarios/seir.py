...
Usage:
    python eval/scenarios/seir.py --seed 42 --n_people 200 --n_steps 50

Output: CSV to stdout with columns step,susceptible,exposed,infected,recovered
...
def run_seir(seed, n_people=200, n_steps=50, n_initial_infected=5,
             beta=0.3, gamma_shape=1.56, gamma_rate=0.53,
             gamma_shift=-2.12, recovery_days=14, exposed_days=3):
    ...
    # Households of 4 + school + company (same as simple_sir)
    households = []
    ...
    results = []
    s_count = sum(1 for i in range(n_people)
                  if not exposed[i] and not infected[i] and not recovered[i])
    e_count = sum(1 for i in range(n_people) if exposed[i])
    i_count = sum(1 for i in range(n_people) if infected[i] and not recovered[i])
    r_count = sum(recovered)
    results.append((0, s_count, e_count, i_count, r_count))

    for step in range(1, n_steps + 1):
        new_exposed = [False] * n_people

        # Transmission: infected → susceptible becomes exposed (not infected)
        for group in all_groups:
            ...
            for pid in group:
                if infected[pid] and not recovered[pid]:
                    ...
                    prob = tg.probability
                    for qid in group:
                        if (not exposed[qid] and not infected[qid]
                                and not recovered[qid] and not new_exposed[qid]):
                            if rng.random() < prob:
        ...
        for i in range(n_people):
            if exposed[i] and not infected[i]:
                if days_exposed[i] >= exposed_days:
                    exposed[i] = False
                    infected[i] = True
                    days_infected[i] = 1
                else:
        ...
        i_count = sum(1 for i in range(n_people) if infected[i] and not recovered[i])
        r_count = sum(recovered)
        results.append((step, s_count, e_count, i_count, r_count))

    return results


def main():
    parser = argparse.ArgumentParser(description="SEIR scenario (Python)")
    ...
    parser.add_argument("--exposed_days", type=int, default=3)
    ...
    results = run_seir(
        ...
        exposed_days=args.exposed_days,
    )

    print("step,susceptible,exposed,infected,recovered")
    for step, s, e, i, r in results:
        print(f"{step},{s},{e},{i},{r}")
